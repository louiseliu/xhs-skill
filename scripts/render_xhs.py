...
def parse_markdown_file(file_path: str) -> dict:
    ...
    body = _resolve_image_paths(body.strip(), base_dir)
...
def _resolve_image_paths(md_text: str, base_dir: str) -> str:
    """Convert relative image paths in Markdown to absolute file:// URIs."""
    def _replace(m):
        alt = m.group(1)
        src = m.group(2)
...
def generate_cover_html(metadata: dict, theme: str, width: int, height: int,
                        base_dir: str = "") -> str:
    """生成封面 HTML。支持两种模式：
    - 有 cover_image: 照片背景 + 文字叠加（小红书主流封面）
    - 无 cover_image: 纯文字大字报风格
    """
    ...
    if cover_image and base_dir and not cover_image.startswith(('http://', 'https://', 'file://', 'data:')):
        cover_image = f'file://{os.path.abspath(os.path.join(base_dir, cover_image))}'
    ...
    if cover_image:
        return _generate_photo_cover_html(
            cover_image, title, subtitle, emoji, title_size, width, height
        )
...
def _generate_photo_cover_html(cover_image: str, title: str, subtitle: str,
                                emoji: str, title_size: int,
                                width: int, height: int) -> str:
    """生成照片背景封面 HTML（小红书主流封面风格）。
    照片铺满背景，底部半透明渐变 + 文字叠加。
    """
    html = f'''<!DOCTYPE html>
<html lang="zh-CN">
    ...
    <style>
        ...
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .cover-text {{
            ...
            left: 0;
            right: 0;
            padding: {int(width * 0.07)}px {int(width * 0.065)}px;
            color: #fff;
        }}

        .cover-text .emoji {{
            font-size: {int(width * 0.09)}px;
            margin-bottom: 8px;
        }}
...
async def render_markdown_to_cards(md_file: str, output_dir: str, 
    ...
    # 生成封面
    if metadata.get('emoji') or metadata.get('title') or metadata.get('cover_image'):
        cover_mode = "照片+文字" if metadata.get('cover_image') else "文字大字报"
        print(f"  📷 生成封面（{cover_mode}）...")
        cover_html = generate_cover_html(metadata, theme, width, height, base_dir)
