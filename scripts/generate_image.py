...
def generate_image_to_image(
    ...
    for img in images:
        ...
        if p.exists():
            ...
            resolved_images.append(str((project_root / p).resolve()))
        else:
            resolved_images.append(img)

    has_local_files = any(os.path.exists(img) for img in resolved_images)

    print(f"🎨 Generating image composition with prompt: {prompt[:60]}...")
    print(f"📐 Input images: {len(resolved_images)}, Model: {model}")
    ...
    try:
        if has_local_files:
            ...
            # Add image files with proper filename and content-type
            file_handles = []
            for img_path in resolved_images:
                if os.path.exists(img_path):
                    ...
                    files.append(('images', (filename, fh, mime)))
                else:
...
def main():
    ...
    for subparser in [text_parser, image_parser]:
        ...
        subparser.add_argument(
            ...
            default="jimeng-4.6",
            choices=["jimeng-4.6"],
            help="Model to use (default: jimeng-4.6)"
        )
        subparser.add_argument(
            "--ratio",
            type=str,
